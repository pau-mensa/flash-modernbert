...
def _load_groups(cfg: dict) -> list[dict]:
    ...
    path = Path(section.get("cache_dir", "showcase/data_cache"))
    ...
    if not path.exists():
        from _common import load_agentir_groups
...
def _build_gradcache_batches(model, groups, cfg, batch_size):
    from packed_training import ColBERTTrainBatch, RoleBatch, build_colbert_train_batch
...
def _gc_step(cfg, variant, model, batch, optimizer):
    from packed_training import gradcache_step
    ...
    with tempfile.TemporaryDirectory(prefix="training-showcase-") as tmp:
        ...
        for family in families:
            for variant in VARIANTS:
                ...
                command = [
                    sys.executable, str(Path(__file__).resolve()), config_path,
                    "--worker", "--family", family, "--variant", variant,
                    "--groups", str(groups_path), "--output", str(output),
                ]
