...
Workflow:
    python scripts/modal_b200_sandbox.py up        # create a persistent sandbox
    python scripts/modal_b200_sandbox.py sync      # upload the current source tree
    python scripts/modal_b200_sandbox.py run -- python -c "import torch; print(torch.cuda.get_device_name(0))"
    python scripts/modal_b200_sandbox.py tests     # run the repository tests
    python scripts/modal_b200_sandbox.py pull REMOTE LOCAL
    python scripts/modal_b200_sandbox.py shell     # interactive shell via `modal container exec`
    python scripts/modal_b200_sandbox.py status    # show the active sandbox
    python scripts/modal_b200_sandbox.py down      # terminate
...
def _require_sandbox() -> modal.Sandbox:
    ...
    if sb is None:
        print(
            "No running sandbox. Run `python scripts/modal_b200_sandbox.py up` first.",
            file=sys.stderr,
        )
        sys.exit(2)
    return sb
...
def cmd_run(argv: list[str]) -> None:
    ...
    proc = sb.exec(
        ...
        "-lc",
        f"cd {workdir_q} && "
        f"export PYTHONPATH={workdir_q}/showcase:{workdir_q}:${{PYTHONPATH:-}} && "
