...
- Modal B200 sandbox managed by `scripts/modal_b200_sandbox.py`
...
```bash
$MODAL_PY scripts/modal_b200_sandbox.py up
$MODAL_PY scripts/modal_b200_sandbox.py sync
```

### 2. Install dependencies

```bash
$MODAL_PY scripts/modal_b200_sandbox.py run -- \
  pip install \
...
```bash
$MODAL_PY scripts/modal_b200_sandbox.py run -- env \
  PYTHONPATH=/workspace/packed-encoders:/workspace/packed-encoders/showcase:/workspace/packed-encoders/flash-maxsim \
  HF_HOME=/workspace/hf-cache \
  PYTHONWARNINGS=ignore \
  python showcase/training_showcase.py \
    showcase/configs/training_b200_single_vector.yml \
...
$MODAL_PY scripts/modal_b200_sandbox.py run -- env \
  ...
  PYTHONWARNINGS=ignore \
  python showcase/training_showcase.py \
    showcase/configs/training_b200_late_interaction_sampled.yml \
...
for name in training_b200_single_vector training_b200_late_interaction; do
  for suffix in .json _iso_loss.png _memory.png; do
    $MODAL_PY scripts/modal_b200_sandbox.py pull \
      /workspace/packed-encoders/showcase/results/${name}${suffix} \
      showcase/results/${name}${suffix}
  done
done

$MODAL_PY scripts/modal_b200_sandbox.py down
```
...
- `showcase/configs/training_b200_single_vector.yml` — run configuration
- `showcase/results/training_b200_single_vector.json` — full results
- `docs/assets/training_b200_single_vector_iso_loss.png` — published fixed-probe
  loss vs optimizer step and vs training wall-clock time
- `docs/assets/training_b200_single_vector_memory.png` — published per-step peak
  CUDA memory (allocated and reserved) for all three variants

### Late interaction

- `showcase/configs/training_b200_late_interaction_sampled.yml` — run
  configuration
- `showcase/results/training_b200_late_interaction.json` — full results
- `docs/assets/training_b200_late_interaction_iso_loss.png` — published fixed-probe
  loss vs optimizer step and vs training wall-clock time
- `docs/assets/training_b200_late_interaction_memory.png` — published per-step peak
  CUDA memory (allocated and reserved) for all three variants
